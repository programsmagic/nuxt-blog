<template>
  <Theme>
    <!-- <HeaderTitle /> -->
    <section class="featured-posts">
      <div class="section-title">
        <h2><span>Featured</span></h2>
      </div>
      <div class="card-columns listfeaturedtag">
        <!-- begin post -->
        <div v-for="(p,i) in randomePost" :key="i" class="card">
          <div class="row">
            <div class="col-md-5 wrapthumbnail">
              <a :href="'/post/'+p.slug">
                <div class="thumbnail" :style="'background-image:url('+base+p.img+');'" />
              </a>
            </div>
            <div class="col-md-7">
              <div class="card-block">
                <h2 class="card-title">
                  <a :href="'/post/'+p.slug">{{ p.title && p.title.substring(0,26) }}</a>
                </h2>
                <h4 class="card-text">
                  {{ p.short_description && p.short_description.substring(0,130)+".." }}
                </h4>
                <div class="metafooter">
                  <div class="wrapfooter">
                    <span class="meta-footer-thumb">
                      <a href="#"><img class="author-thumb" src="https://www.gravatar.com/avatar/e56154546cf4be74e393c62d1ae9f9d4?s=250&amp;d=mm&amp;r=x" alt="Sal"></a>
                    </span>
                    <span class="author-meta">
                      <span class="post-name"><a href="#">Jane</a></span><br>
                      <span class="post-date">22 July 2017</span><span class="dot" /><span class="post-read">6 min read</span>
                    </span>
                    <span class="post-read-more"><a :href="'/post/'+p.slug" title="Read Story"><svg class="svgIcon-use" width="25" height="25" viewbox="0 0 25 25"><path d="M19 6c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v14.66h.012c.01.103.045.204.12.285a.5.5 0 0 0 .706.03L12.5 16.85l5.662 4.126a.508.508 0 0 0 .708-.03.5.5 0 0 0 .118-.285H19V6zm-6.838 9.97L7 19.636V6c0-.55.45-1 1-1h9c.55 0 1 .45 1 1v13.637l-5.162-3.668a.49.49 0 0 0-.676 0z" fill-rule="evenodd" /></svg></a></span>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <!--end post -->
      </div>
    </section>

    <section id="stories" class="recent-posts">
      <div class="section-title">
        <h2><span>All Stories</span></h2>
      </div>
      <div class="card-columns listrecent">
        <!-- begin post -->
        <ListPost v-for="(post,index) in posts.data" :key="index" :post="post" :base="posts.base" />
        <!-- end post -->
      </div>
    </section>
  </Theme>
</template>

<script>
export default {
  name: 'IndexPage',
  components: {
    Theme: () => import('@/layouts/Theme.vue'),
    // HeaderTitle: () => import('@/components/layouts/HeaderTitle.vue'),
    // ListSinglePost: () => import('@/components/posts/ListSinglePost.vue'),
    ListPost: () => import('@/components/posts/ListPost')
  },
  async asyncData ({ $axios, store }) {
    const [posts, random] = await Promise.all([
      (await $axios.$get('https://blog.programsmagic.in/api/paginate')),
      (await $axios.$get('https://blog.programsmagic.in/api/postRrandom')).data
    ])
    // const top6 = await $axios.$get('https://blog.programsmagic.in/api/paginate?page=2&limit=4', {
    //   body: {
    //     page: 2,
    //     limit: 4
    //   }
    // })
    store.commit('posts/setPosts', random)
    return { posts }
  },
  data () {
    return {
      base: 'https://admin.programsmagic.com/public'
    }
  },
  computed: {
    randomePost () {
      return this.$store.state.posts.loadedPosts
    }
  }
}
</script>
<style scoped>

</style>
